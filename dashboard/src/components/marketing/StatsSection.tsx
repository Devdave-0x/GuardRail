...
export function StatsSection() {
  ...
  return (
    <Section id="stats" spacing="tight" background="bg-bg-panel" label="GuardRail by the numbers">
      <dl ref={containerRef} className="grid grid-cols-2 gap-8 lg:grid-cols-4">
        {DERIVED_STATS.map((stat) => (
          <div key={stat.id} data-reveal className="flex flex-col gap-2">
            <dd className="flex items-baseline gap-1 font-mono text-3xl font-bold text-green lg:text-4xl">
              <CountUp to={stat.value} duration={1.2} />
              {stat.suffix && <span className="text-xl lg:text-2xl">{stat.suffix}</span>}
            </dd>
