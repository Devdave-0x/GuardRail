...
export const LaserFlow: React.FC<LaserFlowProps> = ({
  ...
  const mouseSmoothTimeRef = useRef<number>(mouseSmoothTime);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const uniforms: LaserFlowUniforms = {
      ...
      // Reduced motion skips the fade-in and starts fully resolved.
      uFade: { value: hasFadedRef.current || reduced ? 1 : 0 },
    };
    ...
    // Reduced motion: a single resolved frame, and the rAF loop never starts.
    if (reduced) {
      renderer.render(scene, camera);
    } else {
      animate();
    ...
    // The GL context is built once per dpr. Other props flow through the
    // uniform sync effect below.
